...
def add_task_menu():
    ...
    category = input("カテゴリ（任意）: ")
    
    task_id = db.add_task(name, description, due_date, category)
...
def list_tasks_menu():
    ...
    tasks = db.list_tasks(show_completed=False)
    if not tasks:
        print("未完了タスクがありません")
    ...
    for task in tasks:
        ...
        cat = task['category'] if task['category'] else "なし"
        print(f"[{task['id']}] {task['name']}（期日: {due}，カテゴリ: {cat}，累計: {total_time}分）{status}")
    
    
    show_all = input("完了済みも表示しますか？ (y/n): ").lower() == 'y'
    tasks = db.list_tasks(show_completed=True)
    for task in tasks:
        total_time = db.get_task_total_time(task['id'])
        status = "完了" if task['completed'] else "未完了"
        due = task['due_date'] if task['due_date'] else "なし"
